...
source /home/kennyacre/mtconnect_v2/.env

# Configurações
DB_NAME="tn_info_database"
BACKUP_DIR="/home/kennyacre/mtconnect_v2/backups"
...
curl -F chat_id="$TELEGRAM_ID" -F document=@"$ZIP_FILE" -F caption="📦 Backup Automático MTConnect V2 - $TIMESTAMP" "https://api.telegram.org/bot$BOT_TOKEN/sendDocument"

# 4. Limpeza (opcional - mantém apenas os últimos 7 dias localmente)
rm $BACKUP_FILE
find $BACKUP_DIR -name "*.zip" -mtime +7 -exec rm {} \;

echo "Backup concluído e enviado para o Telegram!"
