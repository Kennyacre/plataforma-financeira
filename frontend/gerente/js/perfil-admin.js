...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const elBanco = document.getElementById('pix_banco');
            if (elBanco) elBanco.value = data.pix_banco || "";
        }
    } catch (e) {
    ...
    document.getElementById('form-perfil-admin').addEventListener('submit', async (e) => {
        ...
        if (!user) {
            alert("🚨 Erro: Sessão expirada ou usuário não identificado. Por favor, faça login novamente.");
            return;
        }
        ...
        const payload = {
            ...
            pix_banco: document.getElementById('pix_banco').value
        };
        ...
        try {
            const res = await fetch('/api/admin/atualizar-pix', {
                method: 'POST',
            ...
            if (res.ok) {
                alert("✅ Chave PIX e Titular atualizados com sucesso! Os seus clientes já verão a nova chave.");
            } else {
