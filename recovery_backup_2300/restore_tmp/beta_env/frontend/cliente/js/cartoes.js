...
function desenharCartoes(cartoes) {
    ...
    cartoes.forEach(card => {
        ...
        grid.innerHTML += `
            <div class="credit-card-item" style="background: linear-gradient(135deg, ${card.cor} 0%, #1a1a1a 150%);">
                <button class="btn-delete-card no-theme" onclick="excluirCartao('${card.nome}')" title="Excluir">
                    <span class="material-symbols-rounded no-theme">delete</span>
                </button>
                
                <button class="btn-pay-fatura no-theme" onclick="pagarFatura('${card.nome}')" ${fatura <= 0 ? 'style="opacity: 0.5; cursor: not-allowed;" disabled' : ''}>
                    Pagar Fatura
                </button>

                <div class="cc-bg-pattern no-theme"></div>
                <div class="cc-top no-theme">
                    <div class="cc-chip no-theme"></div>
                    <div class="cc-brand no-theme">${card.nome}</div>
                </div>
                <div class="cc-middle no-theme">
                    <p class="cc-limit-title no-theme">Fatura Atual</p>
                    <p class="cc-limit-value no-theme">${formatarMoeda(fatura)}</p>
                ...
                <div class="cc-bottom no-theme">
                    <div class="cc-date no-theme"><span class="no-theme">Fechamento/Venc.</span>Dia ${card.fechamento} / ${card.vencimento || '--'}</div>
                    <div class="cc-date no-theme" style="text-align: right; color: white !important;"><span class="no-theme">Limite Total</span>${formatarMoeda(limite)}</div>
                </div>
            </div>`;
    });
}
