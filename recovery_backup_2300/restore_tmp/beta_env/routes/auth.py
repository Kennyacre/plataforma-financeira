...
from models.schemas import LoginRequest, PerfilUpdate, ManualRegistrationRequest, TrocarSenhaRequest
import logging
...
def login(request: LoginRequest):
    ...
    try:
        cur.execute("""
            SELECT username, password, role, status, deletado, reset_senha
            FROM usuarios 
        ...
        return {"status": "sucesso", "role": user[2], "username": user[0], "reset_senha": bool(user[5])}
    finally:
...
@router.post("/usuarios/trocar-senha")
def trocar_senha(dados: TrocarSenhaRequest):
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        cur.execute("UPDATE usuarios SET password = %s, reset_senha = FALSE WHERE username = %s AND deletado = FALSE", (dados.nova_senha, dados.username))
        conn.commit()
