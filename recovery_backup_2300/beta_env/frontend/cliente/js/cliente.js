...
function processarDadosParaPizzas(lancamentos, mesAlvo, anoAlvo) {
    ...
    if (totalDinheiro === 0 && totalCartao === 0) {
        ...
        window.myPieChartPgto = new Chart(canvasPgto.getContext('2d'), {
            type: 'doughnut',
            data: { labels: ['Dinheiro/PIX', 'Cartão'], datasets: [{ data: [totalDinheiro, totalCartao], backgroundColor: ['#ffffff', '#666666'], borderWidth: 0 }] },
        ...
        window.myPieChartCat = new Chart(canvasCat.getContext('2d'), {
            type: 'doughnut',
            data: { labels: catLabels, datasets: [{ data: catValues, backgroundColor: ['#ffffff', '#dddddd', '#bbbbbb', '#999999', '#777777', '#555555'], borderWidth: 0 }] },
            options: { responsive: true, maintainAspectRatio: false, cutout: '75%', plugins: { legend: { position: 'bottom', labels: { color: '#94a3b8', font: { size: 11 } } } } }
        });
    }
}
        ...
        container.innerHTML += `
            <div class="card-mini-stat">
                <div class="card-mini-top">
                    <div class="card-mini-name"><span class="material-symbols-rounded" style="color:#ffffff;">credit_card</span> ${card.nome}</div>
                ...
                <div class="progress-mini-bar"><div class="progress-fill" style="width:${percUso}%; background-color:#ffffff;"></div></div>
            </div>
        `;
    });
}
    ...
    try {
        ...
        if (res.ok) {
            ...
            instanceFinanceChart = new Chart(ctx, {
                type: 'bar',
                data: { labels: dados.labels, datasets: [{ label: 'Receitas', data: dados.receitas, backgroundColor: '#ffffff', borderRadius: 4 }, { label: 'Despesas', data: dados.despesas, backgroundColor: '#666666', borderRadius: 4 }] },
