...
function criarUIStatusIA() {
    ...
    style.innerHTML = `
        ...
        .msg.user { background: rgba(255, 255, 255, 0.1); color: white; align-self: flex-end; border: 1px solid rgba(255, 255, 255, 0.2); }
        .msg.waiting { background: #1e293b; border: 1px dashed rgba(255, 255, 255, 0.3); align-self: center; text-align: center; width: 100%; cursor: pointer; }
        .msg.waiting:hover { background: rgba(255, 255, 255, 0.1); }
        .ia-input-area { padding: 10px; background: #1e293b; display: flex; gap: 8px; }
        .ia-input-area input { flex: 1; background: #0f172a; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; padding: 8px 12px; color: white; outline: none; }
        .ia-input-area button { background: #ffffff; border: none; color: #000000; border-radius: 8px; padding: 8px; cursor: pointer; display: flex; align-items: center; }
    `;
...
function addWaitingAction(data) {
    ...
    div.innerHTML = `
        ...
        <button onclick="confirmarAcaoIA('${data.funcao}', ${JSON.stringify(data.parametros).replace(/"/g, '&quot;')})" style="background: #ffffff; color: #000000; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; margin-top: 5px; font-weight: bold;">
            AUTORIZAR AGORA
        </button>
    `;
    box.appendChild(div);
