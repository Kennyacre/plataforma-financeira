...
@router.post("/reset-senha-usuario/{usuario_id}")
def reset_senha_usuario(usuario_id: int):
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        ...
        cur.execute("UPDATE usuarios SET password = %s, reset_senha = TRUE WHERE id = %s", (nova_senha, usuario_id))
        conn.commit()
        
        return {"status": "sucesso", "nova_senha": nova_senha}
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
