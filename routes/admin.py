...
from models.schemas import NovoRevendedor, NovoClienteAdmin, PixUpdate
...
def dashboard_admin():
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM usuarios WHERE solicitacao_renovacao = TRUE")
        total_renovacoes = cur.fetchone()[0]
...
def atualizar_pix(dados: PixUpdate):
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        cur.execute("""
            UPDATE usuarios 
            SET pix_chave = %s, pix_titular = %s, pix_banco = %s 
            WHERE username = %s
        """, (dados.pix_chave, dados.pix_titular, dados.pix_banco, dados.username))
        conn.commit()
        return {"status": "sucesso", "mensagem": "PIX atualizado com sucesso!"}
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
...
@router.get("/solicitacoes-renovacao")
def listar_solicitacoes_renovacao():
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        cur.execute("""
            SELECT id, username, whatsapp, vencimento::text, status 
            FROM usuarios 
            WHERE solicitacao_renovacao = TRUE 
            ORDER BY id DESC
        """)
        rows = cur.fetchall()
        return [{"id": r[0], "username": r[1], "whatsapp": r[2], "vencimento": r[3], "status": r[4]} for r in rows]
    finally:
        cur.close(); conn.close()

@router.post("/confirmar-renovacao/{usuario_id}")
def confirmar_renovacao(usuario_id: int):
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        from datetime import date, timedelta
        # Estendemos 30 dias a partir de HOJE
        novo_vencimento = date.today() + timedelta(days=30)
        
        cur.execute("""
            UPDATE usuarios 
            SET status = 'ativo', 
                solicitacao_renovacao = FALSE, 
                vencimento = %s 
            WHERE id = %s
        """, (novo_vencimento, usuario_id))
        
        conn.commit()
        return {"status": "sucesso", "mensagem": "Usuário renovado e desbloqueado por 30 dias!"}
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
